...
function MessageClient(bubbleName, transportLayerFunctions) {

	var _self = this;

	if(!bubbleName){
		throw 'requires a bubbleName for this MessageClient';
	}
...
MessageClient.prototype.send = function (roomName, messageObject, config) {
	...
	var bubbleTo = (messageObject._destination && messageObject._destination.bubbleTo) || (config && config._destination && config._destination.bubbleTo) || this.bubbleName;
